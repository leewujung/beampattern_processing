...
time_stamp = (0:data.param.extract_call_len_pt-1)/data.mic_data.fs*1e3;
plot(time_stamp,...  % raw time series
     call_no_align + repmat((1:length(data.mic_loc))*shift_gap, data.param.extract_call_len_pt, 1),...
     'color',ones(1,3)*190/255);
hold on
plot(time_stamp,...  % shifted time series
     call_align + repmat((1:length(data.mic_loc))*shift_gap, data.param.extract_call_len_pt, 1),...
     'color',corder(1,:));
short_se_idx = squeeze(data.proc.call_align_short_se_idx(gui_op.current_call_idx,:,:));
for iM=1:length(data.mic_loc)
    if ~isnan(data.mic_loc(iM,1)) && ~any(isnan(short_se_idx(iM,:)))
        want_idx = short_se_idx(iM,:);
